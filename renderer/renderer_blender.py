# A script that uses blender to render a single object by rotation the direction of light around it.
#
...
render.image_settings.color_mode = 'BW' # ('RGB', 'RGBA', ...)
...
for theta in [i for i in range(30, 180, 30)]:
#for theta in [0]:

    #for phi in range(0, args.views):
    for phi in [i for i in range(0, 360, 10)]:    
        #print("Rotation {}, {}".format((stepsize * i), math.radians(stepsize * i)))
        print(f'Lighting theta = {theta}, phi = {phi}')
        #render_file_path = fp + '_r_{0:03d}'.format(int(i * stepsize))
        render_file_path = fp + f'_theta_{theta}_phi_{phi}_'
        ...
        bpy.data.objects['Light'].rotation_euler[2] = math.radians(phi)
        bpy.data.objects['Light'].rotation_euler[1] = math.radians(theta)
        
        bpy.ops.render.render(write_still=True)  # render still

        #cam_empty.rotation_euler[2] += math.radians(stepsize)
        #bpy.data.objects['Light'].rotation_euler[0] += stepsize
